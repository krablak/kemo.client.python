...
class ChatUI:
    def __init__(self, stdscr):
        ...
        userlist_hwyx = (TOP_BAR_HEIGHT, curses.COLS, 0, 0)
        chatbuffer_hwyx = (curses.LINES - 2 -1 -TOP_BAR_HEIGHT, curses.COLS,
                           TOP_BAR_HEIGHT+1, 0)
        chatline_yx = (1, curses.COLS, curses.LINES - 1, 0)
        self.win_userlist = stdscr.derwin(*userlist_hwyx)  # derwin(nlines, ncols, begin_y, begin_x)
    ...
    def resize(self):
        ...
        self.win_userlist.resize(TOP_BAR_HEIGHT, w)
        self.win_chatbuffer.resize(h  - 2 -1 -TOP_BAR_HEIGHT, w)
    ...
    def redraw_ui(self):
        ...
        self.stdscr.hline(TOP_BAR_HEIGHT, 0, "-", w)  # window.hline(y, x, ch, n)
    ...
    def redraw_chatbuffer(self):
        ...
        for i in range(min(h, len(self.linebuffer))):
            line = self.linebuffer[j]
            if "[%s]" % self.username in line:
                color = curses.color_pair(1)
            else:
                #curses.init_pair(1, curses.COLOR_RED, curses.COLOR_WHITE)
                color = curses.color_pair(2)

            self.win_chatbuffer.addstr(i, 0, self.linebuffer[j], color)
            j += 1
    ...
    def _linebuffer_add(self, msg):
        ...
        while len(msg) >= w:  # makes wrapping of messages of window is smaller
